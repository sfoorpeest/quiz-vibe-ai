    ...
    return (
        <div className="min-h-screen bg-slate-50 flex flex-col">
            ...
            <div className="flex-1 max-w-7xl w-full mx-auto p-4 sm:p-6 lg:p-8 flex gap-6 h-[calc(100vh-64px)]">
                ...
                <div className="w-1/3 max-w-sm bg-white border border-slate-200 rounded-3xl overflow-hidden flex flex-col shadow-xl shadow-slate-200/50 hidden md:flex">
                    <div className="p-5 border-b border-slate-100 bg-slate-50/50">
                        ...
                        <div className="mt-4 relative">
                            ...
                            <input 
                                ...
                                className="w-full bg-slate-100 text-sm text-slate-800 placeholder:text-slate-500 rounded-xl pl-9 pr-4 py-2.5 border border-slate-200 focus:outline-none focus:border-blue-500 focus:ring-1 focus:ring-blue-500/20 transition-all"
                            />
                        </div>
                    </div>
                ...
                <div className="flex-1 bg-white border border-slate-200 rounded-3xl shadow-xl shadow-slate-200/50 flex flex-col relative overflow-hidden">
                    {selectedContact ? (
                        <>
                            {/* Header Chat */}
                            <div className="px-6 py-4 border-b border-slate-100 bg-white flex items-center gap-4 z-10">
                                ...
                                {isLoadingHistory ? (
                                    ...
                                    <AnimatePresence initial={false}>
                                        {messages.map((msg, index) => {
                                            ...
                                            // Logic hiển thị thanh phân cách ngày
                                                ...
                                                <React.Fragment key={msg.id || index}>
                                                    {showDateSeparator && (
                                                        <div className="flex justify-center my-6">
                                                            <span className="px-4 py-1.5 rounded-full bg-slate-100 text-slate-500 text-[11px] font-semibold tracking-wider uppercase border border-slate-200/50 shadow-sm">
                                                    ...
                                                    >
                                                        <div className="flex flex-col gap-1 max-w-[75%]">
                                                            ...
                                                            {msg.is_forwarded && (
                                                                <span className={`text-[10px] flex items-center gap-1 ${isMine ? 'text-blue-600 justify-end' : 'text-slate-500'}`}>
                                                            ...
                                                            <div className="flex items-end gap-2">
                                                                ...
                                                                {isFile && !isMine && (
                                                                    <button
                                                                        ...
                                                                        className="opacity-0 group-hover:opacity-100 transition-opacity p-1.5 rounded-full hover:bg-slate-100 text-slate-400 hover:text-slate-600 shrink-0"
                                                                ...
                                                                <div className={`px-4 py-2.5 rounded-2xl shadow-sm ${
                                                                    ...
                                                                    {isFile ? (
                                                                        <div className="flex flex-col gap-2">
                                                                            ...
                                                                            >
                                                                                ...
                                                                                <span className={`text-sm truncate max-w-[160px] ${isMine ? 'text-blue-50' : 'text-slate-700'} group-hover/file:underline`}>
                                                                                    {msg.file_name || 'Tệp đính kèm'}
                                                                                </span>
                                                                                <Download className={`w-4 h-4 shrink-0 ${isMine ? 'text-blue-100' : 'text-slate-400'}`} />
                                                                            </a>
                                                                            {/* Lời nhắn kèm file (nếu có) */}
                                                                            {msg.content && (
                                                                                <p className={`text-[14px] leading-relaxed border-t pt-2 ${isMine ? 'border-white/10' : 'border-slate-200'}`}>{msg.content}</p>
                                                                            )}
                                                                        </div>
                                                                    ) : (
                                                                    ...
                                                                    <div className={`flex items-center gap-1 mt-1 ${isMine ? 'justify-end' : 'justify-start'}`}>
                                                                        <span className={`text-[10px] ${isMine ? 'text-blue-100/80' : 'text-slate-500'}`}>
                            ...
                            <div className="p-4 border-t border-slate-100 bg-white flex flex-col gap-2">
                                ...
                                {selectedFile && (
                                    <div className="flex items-center gap-2 px-3 py-2 bg-slate-50 rounded-xl border border-slate-200 text-sm text-slate-600">
                                        <FileText className="w-4 h-4 text-blue-500 shrink-0" />
                                        <span className="truncate flex-1">{selectedFile.name}</span>
                                        <button onClick={() => { setSelectedFile(null); if (fileInputRef.current) fileInputRef.current.value = ''; }} className="text-slate-400 hover:text-slate-600">
                                            <X className="w-4 h-4" />
                                        </button>
                                    </div>
                                )}
                                ...
                                <form onSubmit={selectedFile ? (e) => { e.preventDefault(); handleSendFile(); } : handleSendMessage} className="flex gap-2">
                                    ...
                                    {(user?.role_id === 2 || user?.role_id === 3) && (
                                        <button
                                            ...
                                            className="w-11 h-11 rounded-full bg-slate-50 border border-slate-200 flex items-center justify-center text-slate-500 hover:text-blue-600 hover:border-blue-500/50 transition-all shrink-0"
                                    ...
                                    <input
                                        ...
                                        className="flex-1 bg-slate-50 text-slate-900 placeholder:text-slate-500 rounded-full px-6 py-3 border border-slate-200 focus:outline-none focus:border-blue-500 focus:ring-1 focus:ring-blue-500/20 transition-all shadow-sm"
                                    />
                        ...
                        <div className="flex-1 flex flex-col items-center justify-center text-slate-400">
                            ...
                            <div className="w-24 h-24 rounded-full bg-slate-50 border border-slate-100 flex items-center justify-center mb-6 shadow-xl shadow-slate-200/50">
                ...
                {forwardingMessage && (
                    ...
                    >
                        ...
                        >
                            ...
                            <div className="mb-4 p-3 bg-slate-50 rounded-xl border border-slate-200 flex items-center gap-3 text-sm">
                            ...
                            <div className="relative mb-3">
                                ...
                                <input
                                    ...
                                    className="w-full bg-slate-50 text-sm text-slate-800 placeholder:text-slate-500 rounded-xl pl-9 pr-4 py-2.5 border border-slate-200 focus:outline-none focus:border-blue-500 focus:ring-1 focus:ring-blue-500/20 transition-all"
                                    autoFocus
                                />
                            </div>
                            ...
                            <div className="max-h-52 overflow-y-auto space-y-1">
                                ...
                                {(forwardSearchQuery.trim() ? forwardSearchResults : contacts).map(contact => (
                                    ...
                                    >
                                        <div className="w-9 h-9 rounded-full bg-blue-50/10 text-blue-600 flex items-center justify-center shrink-0">
