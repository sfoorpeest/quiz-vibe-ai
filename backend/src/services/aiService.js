...
const generateQuizFromAI = async (topic, limit = 5) => {
    const prompt = `Hãy tạo ${limit} câu hỏi trắc nghiệm tiếng Việt về: ${topic}.`;

    try {
        const text = await generateContent(prompt);
        // Fallback JSON nếu AI mock trả về text
        if (!text.includes('[')) throw new Error("Not a JSON");
        ...
        for (let i = 1; i <= limit; i++) {
            mockQuestions.push({
                question: `Câu hỏi trắc nghiệm số ${i} về ${topic}?`,
